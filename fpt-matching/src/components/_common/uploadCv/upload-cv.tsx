...
const UploadCv = ({ blogId }: { blogId: string }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (
        existUpload?.data?.results?.length &&
        existUpload.data.results.length > 0
      ) {
        toast.dismiss(uploaddId);
      ...
      <DialogContent className="sm:max-w-[505px]  min-h-[450px]">
        <DialogHeader>
          ...
          <DialogDescription className="text-base">
            Đây là nơi bạn sẽ nộp ứng tuyển vào{" "}
            {result?.data?.project?.teamName ?? "team"}
          </DialogDescription>
        </DialogHeader>
        <div className="col-auto ">
          <div className="grid grid-cols-1 gap-2 mt-3">
            <div className="flex pt-3">
              <h3>Vui lòng nộp file CV </h3> <span> </span>
              <h3 className="text-red-500">(nếu có)</h3>
            </div>
